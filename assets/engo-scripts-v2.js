          ...
          if( reponse.length > 0 ){
            for (var i = 0; i < reponse.length; i++) {
              ...
              var item              = reponse[i],
                  ...
                  sold_out	       = item.available          ? "" : "<span class='item-sold-out'>Esgotado</span> ",
            ...
            result_element.html('<p class="col-12">Não há resultados para sua pesquisa</p>');
             $('.js_productSearchResults').show();
          }
       }     
    });
    ...
    jQuery( document ).delegate( search_input_id ,'keyup',function( event ){
      ...
      ajax_timeout = setTimeout(function() {
        if( keyword.length >= 2 ){
            ...
            result_element.html('<p class="col-12">Você precisa colocar pelo menos 2 caracteres</p>');
        } 
      },300);       
    }); 
...
jQuery(document).ready(function($) {
  ...
  function cookiepopup(){
    ...
    $(window).on('load', function(){
      setTimeout(function(){ $('.jsengo_popup').show(); }, 300);

      $('.jsclosepoup').on('click', function(e) {
        e.preventDefault();
        $('.jsengo_popup').hide();
